...
public class BossBullet extends Sprite {
    ...
    public BossBullet(Boss boss, float mouseX, float mouseY, int size,boolean flipped){

        super(new Texture(Gdx.files.internal("boss/bossBullet.png")));
        setBounds(0, 0, 25 / MyGdxGame.PPM, 25 / MyGdxGame.PPM);


        BodyDef bdef = new BodyDef();
        bdef.position.set((flipped)?boss.getBody().getPosition().x+boss.getWidth()/3:boss.getBody().getPosition().x-boss.getWidth()/3, boss.getBody().getPosition().y + 4 / MyGdxGame.PPM);
        ...
        fdef.shape = shape;
        body.createFixture(fdef).setUserData("bossBullet");
