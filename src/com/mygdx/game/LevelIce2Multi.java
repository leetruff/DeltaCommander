 ...
 */
public class LevelIce2Multi extends LevelIce2 {
    ...
    public LevelIce2Multi(MyGdxGame game, int health, int lives, int ammo, int score, int laserAmmo, int tripleAmmo,Server server, Connection con){
        super(game,health,lives,ammo,score,laserAmmo,tripleAmmo);
        this.server = server;
        connection = con;
        isServer = true;
    }
    public LevelIce2Multi(MyGdxGame game, int health, int lives, int ammo, int score, int laserAmmo, int tripleAmmo,Client client){
        super(game,health,lives,ammo,score,laserAmmo,tripleAmmo);
        this.client=client;
        isServer=false;
    }
    public LevelIce2Multi(MyGdxGame game, Server server, Connection con) {
        super(game);
        this.server = server;
        connection = con;
        isServer = true;
    ...
    public LevelIce2Multi(MyGdxGame game, Client client) {
        super(game);
        this.client = client;
        isServer = false;
            ...
            if (!super.gameOver&&!super.levelCompleted){
                if (Application.ApplicationType.Desktop == Gdx.app.getType()) {
                    ...
                    if (!super.paused) {
                        ...
                        //Sprung
                        if ((Gdx.input.isKeyJustPressed(Input.Keys.W) || Gdx.input.isKeyJustPressed(Input.Keys.SPACE)) && super.hero.getState() != Hero.State.JUMPING && super.hero.getState() != Hero.State.FALLING)
                            super.hero.getBody().applyLinearImpulse(new Vector2(0, 5.5f), super.hero.getBody().getWorldCenter(), true);

                        //Nach rechts laufen
                        if (Gdx.input.isKeyPressed(Input.Keys.D) && super.hero.getBody().getLinearVelocity().x <= 3) {
                        ...
                        //Nach links laufen
                        if (Gdx.input.isKeyPressed(Input.Keys.A) && super.hero.getBody().getLinearVelocity().x >= -3) {
                        ...
                        if (!Gdx.input.isKeyPressed(Input.Keys.D) && !Gdx.input.isKeyPressed(Input.Keys.A) && super.hero.getState() == Hero.State.RUNNING)
                ...
                else
                {
                    if(Gdx.input.isKeyJustPressed(Input.Keys.ENTER))
                    {
                        super.levelCompletedMenu.continueButton.setX(Gdx.graphics.getWidth()/2-levelCompletedMenu.continueButton.getWidth()/2);
                        super.levelCompletedMenu.setLabelX(Gdx.graphics.getWidth()/2-levelCompletedMenu.levelCompleted.getWidth()/2);
                    }
                    Gdx.input.setInputProcessor(super.levelCompleteStage);
                }
    ...
    public void render(float delta)
    {
        ...
        if(restart)
        {
            ...
            super.game.setScreen(new LevelIce2Multi(super.game,client));
        }
    ...
    public void stageCompleted(){
        ...
        if(isServer) {
            game.setScreen(new LevelIce3Multi(game, hero.getHealth(), hero.getLives(), hero.getAmmo(), hero.getScore(), hero.getLaserAmmo(), hero.getTripleAmmo(),server,connection));
        }
        else
        {
            game.setScreen(new LevelIce3Multi(game, hero.getHealth(), hero.getLives(), hero.getAmmo(), hero.getScore(), hero.getLaserAmmo(), hero.getTripleAmmo(),client));
    ...
    public void restart()
    {
        if(isServer)
        {
            ...
            super.game.setScreen(new LevelIce2Multi(super.game,server,connection));
        }
