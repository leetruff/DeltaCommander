...
public class Level3 extends GameScreen {
	...
	/**
	 * Konstruktor zum Erzeugen eines Level3-Objekts
	 ...
	 */
	public Level3(MyGdxGame game, int health, int lives, int ammo, int score, int laserAmmo, int tripleAmmo){
		this(game);
	...
	public void show() {
		...
		map = maploader.load("level/level3.tmx");
		...
		shieldPU1 = new ShieldPU(world, 2603, 326);
	   ...
	   pauseMenu.menuButton.addListener(new ChangeListener() {
		...
		platform1 = new MovingPlatform(hero, world, new Vector2(700, 375), new Vector2(1040, 375), new Vector2(1, 0));
		platformList.add(platform1);
		platformList.add(new MovingPlatform(hero,world, new Vector2(2115,525),new Vector2(2342,525),new Vector2(1,0)));
		...
		enemyTowerList.add(new EnemyTower(world,hero,1515,365,false,game));
		enemyTowerList.add(new EnemyTower(world,hero,1826,365,false,game));

		enemySoldierList = new ArrayList<EnemySoldier>();
		enemySoldierList.add(new EnemySoldier(world,hero,this,2426,174,game));
		enemySoldierList.add(new EnemySoldier(world,hero,this,2928,174,game));
	...
	public void update(float delta){
		...
		{
			boss.update();
			if(hero.b2body.getPosition().x>31.46f)
			{
				hero.b2body.setTransform(31.46f,hero.b2body.getPosition().y,0);
			}
		}
		if(hero.b2body.getPosition().x>27.3&&!bossfight&&!levelCompleted)
		{
			game.playBossTheme1();
			bossfight=true;
			boss= new Boss(world,hero,29.54f,6f,gamecam,game);
		}
  	...
  	public class WorldContactListener implements ContactListener {
  		...
  		public void beginContact(Contact contact) {
  			...
  			if(fixA.getUserData() == "hero" || fixB.getUserData() == "hero"){
				...
				{
					if(boss.currentState==Boss.State.FALLING&&hero.b2body.getPosition().y<1.2f)
					{
	...
	public void respawn()
	{
		...
		if(hero.lives==0 && gameOver == false)
		{
			...
			if(bossfight)
			{
				for (BossBullet b:boss.getBulletList())
				{
					world.destroyBody(b.getBody());
					b.getBody().setUserData(null);
					b.setBody(null);
				}
				boss.setBulletList(new ArrayList<BossBullet>());
			}
	...
	public void initCheckpoints()
	{
		...
		checkpoints[1]= new Checkpoint(15.61f,1.16f-hero.getHeight()/2);
		checkpoints[2]= new Checkpoint(23.24f,1.164f-hero.getHeight()/2);
	...
	private void stageCompleted(){
		...
		XMLInteraction.saveScore("level3", game.getUsername(), hero.getScore());
