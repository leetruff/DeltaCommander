...
/**
 * Screen: Level 2 der Antarctica Welt
 * @Leveldesign Hermann
 * @author Fabian, Lars, Hermann, Tim
 */
public class LevelIce2 extends GameScreen {

	//Zeit die zu unterbieten ist in Sekunden:
	final int timeToBeat = 90;
	 ...
	 */
	public LevelIce2(MyGdxGame game, int health, int lives, int ammo, int score, int laserAmmo, int tripleAmmo){
		this(game);
	...
	public void show() {
		...
		map = maploader.load("level/iceLevel2.tmx");
		...
		healthPU1 = new HealthPU(world, 4094, 1046);
		powerUpList.add(healthPU1);
		shieldPU1 = new ShieldPU(world, 8114, 1645);
		powerUpList.add(shieldPU1);
		lifePU1 = new LifePU(world, 7176, 1389);
	   ...
	   pauseMenu.saveButton.addListener(new ChangeListener() {
	       ...
	       public void changed(ChangeEvent event, Actor actor) {
			   game.playButtonClicked();
				XMLInteraction.saveGame(hero, "levelice2");
	       }});
	   pauseMenu.menuButton.addListener(new ChangeListener() {
		...
		enemyTowerList.add(new EnemyTower(world,hero,3711,911,false,game));
		enemyTowerList.add(new EnemyTower(world,hero,4474,1170,false,game));
		enemyTowerList.add(new EnemyTower(world,hero,5375,911,false,game));
		enemyTowerList.add(new EnemyTower(world,hero,6270,910,false,game));
		enemyTowerList.add(new EnemyTower(world,hero,7550,1295,false,game));


		enemySoldierList = new ArrayList<EnemySoldier>();
		enemySoldierList.add(new EnemySoldier(world,hero,this,1282,409,game));
		enemySoldierList.add(new EnemySoldier(world,hero,this,1613,540,game));
		enemySoldierList.add(new EnemySoldier(world,hero,this,1857,540,game));
		...
		enemySoldierList.add(new EnemySoldier(world,hero,this,8291,1176,game));
		enemySoldierList.add(new EnemySoldier(world,hero,this,8732,1176,game));
  		...
  		public void beginContact(Contact contact) {
  			...
  			if(fixA.getUserData() == "bullet" || fixB.getUserData() == "bullet" && fixA.getUserData() != fixB.getUserData())
  			{
  				...
  				if((!((Bullet)bullet.getBody().getUserData()).isHitObject()) && object.getUserData() != "hero" && object.getUserData() != null)
  				{
					...
					{
						((Boss)object.getBody().getUserData()).setLife(((Boss)object.getBody().getUserData()).getLife()-((Bullet)bullet.getBody().getUserData()).getDamage());
						((Bullet)bullet.getBody().getUserData()).setHitObject(true);
	...
	public void initCheckpoints()
	{
		...
		checkpoints[0]= new Checkpoint(1f,1-hero.getHeight()/2);
		checkpoints[1]= new Checkpoint(17.26f,3.57f-hero.getHeight()/2);
		checkpoints[2]= new Checkpoint(44.2f,6.99f-hero.getHeight()/2);
		checkpoints[3]= new Checkpoint(66.52f,9.55f-hero.getHeight()/2);
	...
	public void stageCompleted(){
		...
		XMLInteraction.saveScore("levelice2", game.getUsername(), hero.getScore());
		hero.updateSavedScore();
		game.setScreen(new LevelIce3(game, hero.getHealth(), hero.getLives(), hero.getAmmo(), hero.getScore(), hero.getLaserAmmo(), hero.getTripleAmmo()));
	}
	/**
	...
	public void restart()
	{
		game.setScreen(new LevelIce2(game));
	}
