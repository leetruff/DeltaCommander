...
/**
 * Screen: Level 3 der Antarctica Welt
 * @Leveldesign Hermann
 * @author Fabian, Lars, Hermann, Tim
 */
public class LevelIce3 extends GameScreen {
	...
	/**
	 * Konstruktor zum Erzeugen eines LevelIce3-Objekts
	 * @param game Startklassen-Instanz: Zugriff auf globale Ressourcen, haelt aktuellen Screen
	 */
	public LevelIce3(MyGdxGame game){
		this.game = game;
		this.game.playLevelThemeW2();
	 ...
	 */
	public LevelIce3(MyGdxGame game, int health, int lives, int ammo, int score, int laserAmmo, int tripleAmmo){
		this(game);
	...
	public void show() {
		...
		map = maploader.load("level/iceLevel3.tmx");
		...
		//healthPU1 = new HealthPU(world, 1658, 618);
		//powerUpList.add(healthPU1);
		shieldPU1 = new ShieldPU(world, 3297, 760);
		powerUpList.add(shieldPU1);
		lifePU1 = new LifePU(world, 1577, 740);
		powerUpList.add(lifePU1);
		powerUpList.add(new LaserPU(world, 5503,284));
	   ...
	   pauseMenu.saveButton.addListener(new ChangeListener() {
	       ...
	       public void changed(ChangeEvent event, Actor actor) {
				game.playButtonClicked();
				XMLInteraction.saveGame(hero, "level3");
	       }});
	   pauseMenu.menuButton.addListener(new ChangeListener() {
		...
		enemyHomingList.add(new EnemyHoming(world, hero, 5200, 730, game));

		enemyTowerList = new ArrayList<EnemyTower>();
		enemyTowerList.add(new EnemyTower(world,hero,1477,270,false,game));
		enemyTowerList.add(new EnemyTower(world,hero,1730,270,false,game));
		enemyTowerList.add(new EnemyTower(world,hero,3771,464,false,game));
		enemyTowerList.add(new EnemyTower(world,hero,4030,464,false,game));

		enemySoldierList = new ArrayList<EnemySoldier>();
		enemySoldierList.add(new EnemySoldier(world,hero,this,1474,601,game));
		enemySoldierList.add(new EnemySoldier(world,hero,this,3260,406,game));
		enemySoldierList.add(new EnemySoldier(world,hero,this,5762,408,game));
		enemySoldierList.add(new EnemySoldier(world,hero,this,1871,534,game));
		enemySoldierList.add(new EnemySoldier(world,hero,this,5758,856,game));
		enemySoldierList.add(new EnemySoldier(world,hero,this,5371,729,game));
		...
		hero.b2body.setTransform(1,2,0);
		background = new Sprite(new Texture("level/iceBackground.png"));
	...
	public void update(float delta){
		...
		{
			boss.update();
			if(hero.b2body.getPosition().x<41.8f)//Abfrage für Randfälle
			{
				hero.b2body.setTransform(41.8f,hero.b2body.getPosition().y,0);
			}
			else if(hero.b2body.getPosition().x>50.1f)
			{
				hero.b2body.setTransform(50.1f,hero.b2body.getPosition().y,0);

			}
		}
		if(hero.b2body.getPosition().x>46&&!bossfight&&!levelCompleted)
		{
			game.playBossTheme3();
			bossfight=true;
			boss= new TankBoss(world,hero,7519/MyGdxGame.PPM,344/MyGdxGame.PPM,gamecam,game,43,50);
		}
		...
		if(bossfight && boss != null){
			if(boss.getLife()>0){
				bossHealthBar.setSize(bossBarWidth*((float)boss.getLife()/3000), bossHealthBar.getHeight());
			}
			else{
		...
		if(hero.b2body.getPosition().y*MyGdxGame.PPM>500)
			gamecam.position.y=2.4f+(hero.b2body.getPosition().y-(500f/MyGdxGame.PPM));
			...
			if (Application.ApplicationType.Desktop == Gdx.app.getType()) {
				...
				if (!paused) {
					...
					if (Gdx.input.isKeyJustPressed(Input.Keys.ENTER)) {
						...
						System.out.println("X: " + hero.b2body.getPosition().x*MyGdxGame.PPM + " Y: " + hero.b2body.getPosition().y*MyGdxGame.PPM);
					}
				}
	...
	public void render(float delta) {
		...
		if(bossfight) {
			for (int i = 0; i < boss.getBulletList().size(); i++) {
				boss.getBulletList().get(i).draw(batch);
			}
		}
		...
		if(bossfight&&boss.defeated==true)
		{
			bossfight=false;
			levelCompleted=true;
			game.playLevelUp();
  	...
  	public class WorldContactListener implements ContactListener {
  		...
  		public void beginContact(Contact contact) {
  			...
  			if(fixA.getUserData() == "hero" || fixB.getUserData() == "hero"){
				...
				//Kollision mit fallendem Boss
				else if(object.getUserData()=="tankboss")
				{
					if(boss.currentState==(TankBoss.State.SPRINTING))
					{
  			...
  			if(fixA.getUserData() == "bullet" || fixB.getUserData() == "bullet" && fixA.getUserData() != fixB.getUserData())
  			{
  				...
  				if((!((Bullet)bullet.getBody().getUserData()).isHitObject()) && object.getUserData() != "hero" && object.getUserData() != null)
  				{
					...
					{
						((TankBoss)object.getBody().getUserData()).setLife(((TankBoss)object.getBody().getUserData()).getLife()-((Bullet)bullet.getBody().getUserData()).getDamage());
						((Bullet)bullet.getBody().getUserData()).setHitObject(true);
			...
			if(fixA.getUserData() == "joinherobullet" || fixB.getUserData() == "joinherobullet" && fixA.getUserData() != fixB.getUserData())
			{
				...
				if(object.getUserData() != null&&(!((JoinHeroBullet)bullet.getBody().getUserData()).isHitObject()) && object.getUserData() != "joinhero" )
				{
					...
					{
						((TankBoss)object.getBody().getUserData()).setLife(((TankBoss)object.getBody().getUserData()).getLife()-((JoinHeroBullet)bullet.getBody().getUserData()).getDamage());
						((JoinHeroBullet)bullet.getBody().getUserData()).setHitObject(true);
			...
			if(fixA.getUserData() == "laserbullet" || fixB.getUserData() == "laserbullet" && fixA.getUserData() != fixB.getUserData())
			{
				...
				if((!((Laserbullet)bullet.getBody().getUserData()).isHitObject()) && object.getUserData() != "hero" && object.getUserData() != null)
				{
					...
					{
						((TankBoss)object.getBody().getUserData()).setLife(((TankBoss)object.getBody().getUserData()).getLife()-((Laserbullet)bullet.getBody().getUserData()).getDamage());
						((Laserbullet)bullet.getBody().getUserData()).setHitObject(true);
			...
			if(fixA.getUserData() == "tankbossBullet" || fixB.getUserData() == "tankbossBullet" && fixA.getUserData() != fixB.getUserData())
			{
				Fixture bullet = fixA.getUserData() == "tankbossBullet" ? fixA : fixB;
				Fixture object = bullet == fixA ? fixB : fixA;

				if((!((TankBossBullet)bullet.getBody().getUserData()).isHitObject()) && object.getUserData() != "tankboss" && object.getUserData() != null)
				{
					//Kollision mit Hero
					if(object.getUserData() == "hero")
					{
						...
						if(!shield.isActive() && !hero.isInvulnerable())
						{
							hero.setHealth(hero.getHealth()-((TankBossBullet)bullet.getBody().getUserData()).getDamage());
							hero.setDamageEffect(0.25f);
						}
						((TankBossBullet)bullet.getBody().getUserData()).setHitObject(true);
					}
					//Kollision anderem Objekt, das keine Projektile durchlassen soll
					else if(object.getUserData() == "map" || object.getUserData() instanceof MovingPlatform)
					{
	...
	public void respawn()
	{
		...
		if(hero.lives==0 && gameOver == false)
		{
			...
			if(bossfight)
			{
				for (TankBossBullet b:boss.getBulletList())
				{
					world.destroyBody(b.getBody());
					b.getBody().setUserData(null);
					b.setBody(null);
				}
				boss.setBulletList(new ArrayList<TankBossBullet>());
			}
	...
	public void initCheckpoints()
	{
		...
		checkpoints[0]= new Checkpoint(0.8f,1.87f-hero.getHeight()/2);
		checkpoints[1]= new Checkpoint(17.17f,1.87f-hero.getHeight()/2);
		checkpoints[2]= new Checkpoint(31.52f,3.591f-hero.getHeight()/2);
		checkpoints[3]= new Checkpoint(43f,4f-hero.getHeight()/2);
	...
	private void stageCompleted(){
		...
		XMLInteraction.saveScore("levelice3", game.getUsername(), hero.getScore());
	...
	public void restart()
	{
		game.setScreen(new LevelIce3(game));
	}
