 ...
 */
public class Level3Multi extends Level3
{
    ...
    public Level3Multi(MyGdxGame game, int health, int lives, int ammo, int score, int laserAmmo, int tripleAmmo, Server server, Connection con){
        super(game,health,lives,ammo,score,laserAmmo,tripleAmmo);
        this.server = server;
        connection = con;
        isServer = true;
    }
    public Level3Multi(MyGdxGame game, int health, int lives, int ammo, int score, int laserAmmo, int tripleAmmo, Client client){
        super(game,health,lives,ammo,score,laserAmmo,tripleAmmo);
        this.client=client;
        isServer=false;
    }
    public Level3Multi(MyGdxGame game, Server server, Connection con) {
        super(game);
        this.server = server;
        connection = con;
        isServer = true;
    ...
    public Level3Multi(MyGdxGame game, Client client) {
        super(game);
        this.client = client;
        isServer = false;
        ...
        if(isServer) {
            if (!super.gameOver&&!super.levelCompleted){
                if (Application.ApplicationType.Desktop == Gdx.app.getType()) {
                    ...
                    //Pausieren
                    if(Gdx.input.isKeyJustPressed(Input.Keys.BACK) || Gdx.input.isKeyJustPressed(Input.Keys.MENU) || Gdx.input.isKeyJustPressed(Input.Keys.ESCAPE)){
    ...
    public void render(float delta)
    {
        ...
        if(restart)
        {
            super.game.setScreen(new Level3Multi(super.game,client));
        }
    ...
    public void restart()
    {
        if(isServer)
        {
            ...
            super.game.setScreen(new Level3Multi(super.game,server,connection));
        }
