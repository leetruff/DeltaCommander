 ...
 */
public class LevelScifi2Multi extends LevelScifi2 {
    ...
    public LevelScifi2Multi(MyGdxGame game, int health, int lives, int ammo, int score, int laserAmmo, int tripleAmmo,Server server, Connection con){
        super(game,health,lives,ammo,score,laserAmmo,tripleAmmo);
        this.server = server;
        connection = con;
        isServer = true;
    }
    public LevelScifi2Multi(MyGdxGame game, int health, int lives, int ammo, int score, int laserAmmo, int tripleAmmo,Client client){
        super(game,health,lives,ammo,score,laserAmmo,tripleAmmo);
        this.client=client;
        isServer=false;
    }
    ...
    public LevelScifi2Multi(MyGdxGame game, Server server, Connection con) {
        super(game);
        this.server = server;
        connection = con;
        isServer = true;
    ...
    public LevelScifi2Multi(MyGdxGame game, Client client) {
        super(game);
        this.client = client;
        isServer = false;
    ...
    public void render(float delta)
    {
        ...
        if(restart)
        {
            ...
            super.game.setScreen(new LevelScifi2Multi(super.game,client));
        }
    ...
    public void stageCompleted(){
        ...
        XMLInteraction.saveScore("levelscifi2", game.getUsername(), hero.getScore());
        hero.updateSavedScore();
        if(isServer) {
            game.setScreen(new LevelScifi3Multi(game, hero.getHealth(), hero.getLives(), hero.getAmmo(), hero.getScore(), hero.getLaserAmmo(), hero.getTripleAmmo(),server,connection));
        }
        else
        {
            game.setScreen(new LevelScifi3Multi(game, hero.getHealth(), hero.getLives(), hero.getAmmo(), hero.getScore(), hero.getLaserAmmo(), hero.getTripleAmmo(),client));
    ...
    public void restart()
    {
        if(isServer)
        {
            ...
            super.game.setScreen(new LevelScifi2Multi(super.game,server,connection));
        }
