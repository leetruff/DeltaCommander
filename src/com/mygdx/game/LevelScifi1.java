...
/**
 * Screen: Level 1 der SciFi Welt
 * @Leveldesign Fabian
 * @author Fabian, Lars, Hermann, Tim
 */
public class LevelScifi1 implements Screen {

	//Zeit die zu unterbieten ist in Sekunden:
	final int timeToBeat = 84;
	 ...
	 */
	public LevelScifi1(MyGdxGame game, int health, int lives, int ammo, int score, int laserAmmo, int tripleAmmo){
		this(game);
	...
	public void show() {
		...
		map = maploader.load("level/scifiLevel1.tmx");
		renderer = new OrthogonalTiledMapRenderer(map, 1 / (MyGdxGame.PPM * 4));

		b2dr = new Box2DDebugRenderer();
		
		
		hero.b2body.setTransform(300 / MyGdxGame.PPM, 200 / MyGdxGame.PPM, 0);
	   ...
	   pauseMenu.saveButton.addListener(new ChangeListener() {
	       ...
	       public void changed(ChangeEvent event, Actor actor) {
				game.playButtonClicked();
				XMLInteraction.saveGame(hero, "levelscifi1");
	       }});
	   pauseMenu.menuButton.addListener(new ChangeListener() {
		...
		homing1 = new EnemyHoming(world, hero, 2.878235f * MyGdxGame.PPM, 4.349994f * MyGdxGame.PPM, game, 220);
		enemyHomingList.add(homing1);
		
		homing2 = new EnemyHoming(world, hero, 3.378235f * MyGdxGame.PPM, 4.349994f * MyGdxGame.PPM, game, 220);
		enemyHomingList.add(homing2);
		
		homing3 = new EnemyHoming(world, hero, 3.878235f * MyGdxGame.PPM, 4.349994f * MyGdxGame.PPM, game, 220);
		enemyHomingList.add(homing3);
		
		homing4 = new EnemyHoming(world, hero, 4.378235f * MyGdxGame.PPM, 4.349994f * MyGdxGame.PPM, game, 220);
		enemyHomingList.add(homing4);
		
		homing5 = new EnemyHoming(world, hero, 27.878235f * MyGdxGame.PPM, 5.149994f * MyGdxGame.PPM, game);
		enemyHomingList.add(homing5);
		
		homing6 = new EnemyHoming(world, hero, 28.78235f * MyGdxGame.PPM, 6.049994f * MyGdxGame.PPM, game);
		enemyHomingList.add(homing6);
		
		enemySoldierList = new ArrayList<EnemySoldier>();
		
		soldier1 = new EnemySoldier(world, hero, 0.278235f * MyGdxGame.PPM, 4.5049994f * MyGdxGame.PPM, game);
		enemySoldierList.add(soldier1);
		
		soldier2 = new EnemySoldier(world, hero, 4.275579f * MyGdxGame.PPM, 5.505708f * MyGdxGame.PPM, game);
		enemySoldierList.add(soldier2);
		
		soldier3 = new EnemySoldier(world, hero, 4.49139f * MyGdxGame.PPM, 5.716661f * MyGdxGame.PPM, game);
		enemySoldierList.add(soldier3);
		
		soldier4 = new EnemySoldier(world, hero, 5.288941f * MyGdxGame.PPM, 5.716657f * MyGdxGame.PPM, game);
		enemySoldierList.add(soldier4);
		
		
		
		enemyTowerList = new ArrayList<EnemyTower>();
		tower1 = new EnemyTower(world, hero, 8.7333333f * MyGdxGame.PPM, 1.3866666f * MyGdxGame.PPM, false, game);
		enemyTowerList.add(tower1);
		
		tower2 = new EnemyTower(world, hero, 8.7333333f * MyGdxGame.PPM, 6.506657f * MyGdxGame.PPM, false, game);
		enemyTowerList.add(tower2);
		
		tower3 = new EnemyTower(world, hero, 16.824869f * MyGdxGame.PPM,  6.09657f * MyGdxGame.PPM, false, game);
		enemyTowerList.add(tower3);
		
		tower4 = new EnemyTower(world, hero, 28.7333333f * MyGdxGame.PPM, 4.788f * MyGdxGame.PPM, false, game);
		enemyTowerList.add(tower4);
		
		tower5 = new EnemyTower(world, hero, 29.7f * MyGdxGame.PPM, 5.64f * MyGdxGame.PPM, false, game);
		enemyTowerList.add(tower5);
		
		tower6 = new EnemyTower(world, hero, 30.5f * MyGdxGame.PPM,  6.5f * MyGdxGame.PPM, false, game);
		enemyTowerList.add(tower6);
		...
		healthPU1 = new HealthPU(world, 18.824869f * MyGdxGame.PPM,  6.39657f * MyGdxGame.PPM);
		powerUpList.add(healthPU1);
		
		tripleshotPU1 = new TripleshotPU(world, 17.924869f * MyGdxGame.PPM,  6.539657f * MyGdxGame.PPM);
	...
	public void update(float delta){
		...
		if(hero.getBody().getPosition().y * MyGdxGame.PPM > MyGdxGame.V_HEIGHT / 2)
		...
		//Homing-Gegner, die besiegt wurden aus der Liste entfernen und den zugehoerigen Body zerstoeren
		for(int i = 0; i < enemyHomingList.size(); i++){
	...
	public void respawn()
	{
		...
		if(hero.lives==0 && gameOver == false)
		{
			...
			if(hero.getBody().getPosition().y * MyGdxGame.PPM > 500)
			gamecam.position.y = hero.getBody().getPosition().y;
	...
	public void initCheckpoints()
	{
		...
		checkpoints[0]= new Checkpoint(195 / MyGdxGame.PPM, 194 / MyGdxGame.PPM);
		checkpoints[1]= new Checkpoint(1793 / MyGdxGame.PPM, 832 / MyGdxGame.PPM);
		checkpoints[2]= new Checkpoint(4938 / MyGdxGame.PPM, 1089 / MyGdxGame.PPM);
	...
	public void stageCompleted(){
		...
		XMLInteraction.saveScore("levelscifi1", game.getUsername(), hero.getScore());
		hero.updateSavedScore();
		game.setScreen(new LevelScifi2(game, hero.getHealth(), hero.getLives(), hero.getAmmo(), hero.getScore(), hero.getLaserAmmo(), hero.getTripleAmmo()));
	}
	/**
	...
	public void restart()
	{
		game.setScreen(new LevelScifi1(game));
	}
